...
public class TypeIdentifService {
  ...
  /**
   * Partially update a typeIdentif.
   *
   * @param typeIdentif the entity to update partially.
   * @return the persisted entity.
   */
  public Optional<TypeIdentif> partialUpdate(TypeIdentif typeIdentif) {
    log.debug("Request to partially update TypeIdentif : {}", typeIdentif);

    return typeIdentifRepository
      .findById(typeIdentif.getId())
      .map(
        existingTypeIdentif -> {
          if (typeIdentif.getIdentifier() != null) {
            existingTypeIdentif.setIdentifier(typeIdentif.getIdentifier());
          }
          if (typeIdentif.getDefaultIdentifier() != null) {
            existingTypeIdentif.setDefaultIdentifier(typeIdentif.getDefaultIdentifier());
          }

          return existingTypeIdentif;
        }
      )
      .map(typeIdentifRepository::save);
  }
